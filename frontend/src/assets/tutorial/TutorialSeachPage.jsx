...
const LS_TUT_ORDER = 'tutorialOrder';

// Simple inline confirm dialog (global blue modal card)
...
export default function AllTutorialsPage() {
    ...
    useEffect(() => {
        const qRef = query(collection(db, 'tutorials'), where('visibility', '==', 'public'));
        const unsub = onSnapshot(
            ...
            (err) => {
                console.error('tutorials onSnapshot error:', err);
                setFsError(err?.message || 'Failed to load tutorials.');
            }
        );
        return () => unsub();
    }, []);
    ...
    const savedOrder = useMemo(() => {
        try { return JSON.parse(localStorage.getItem(LS_TUT_ORDER) || '[]'); }
        catch { return []; }
    }, []);
    ...
    const persistOrder = (ids) => localStorage.setItem(LS_TUT_ORDER, JSON.stringify(ids));
    ...
    async function deleteTutorialAndAssets(tutorialId) {
        // 1) subcollections
        try {
            const commentsSnap = await getDocs(collection(db, 'tutorials', tutorialId, 'comments'));
            await Promise.allSettled(commentsSnap.docs.map((d) => deleteDoc(d.ref)));
        } catch (e) { console.warn('delete comments failed', e); }
        try {
            const ratingsSnap = await getDocs(collection(db, 'tutorials', tutorialId, 'ratings'));
            await Promise.allSettled(ratingsSnap.docs.map((d) => deleteDoc(d.ref)));
        } catch (e) { console.warn('delete ratings failed', e); }

        // 2) storage files under tutorials/{id}
        try {
            const folder = sref(storage, `tutorials/${tutorialId}`);
    ...
    const [confirm, setConfirm] = useState({ open: false, tutorialId: null, title: '', detail: '' });
    ...
    const openDeleteConfirm = (tutorial) => {
        setConfirm({
            ...
            detail:
                ...
                `• Any images under /tutorials/${tutorial.id} in Storage`,
        });
        setAnyModalOpen(true);
    };
    const closeConfirm = () => { setConfirm({ open: false, tutorialId: null, title: '', detail: '' }); setAnyModalOpen(false); };

    const doConfirmedDelete = async () => {
        const tutorialId = confirm.tutorialId;
        closeConfirm();
        if (!tutorialId) return;

        try {
            // optimistic UI
            setFsItems((prev) => prev.filter((x) => x.id !== tutorialId));
            setOrderedIds((prev) => prev.filter((x) => x !== tutorialId));
            await deleteTutorialAndAssets(tutorialId);
            showToast('Tutorial deleted.', true);
        } catch (err) {
            console.error('Delete tutorial failed', err);
            showToast('Failed to delete tutorial.', false);
        }
    };
    ...
    return (
        <div className="container py-4 lobster-regular">
            <div className="d-flex flex-wrap justify-content-between align-items-end gap-3 mb-3">
                <h3 className="mb-0">Find Tutorials</h3>
            ...
            {orderedIds.length === 0 && (
                <div className="alert alert-info">No tutorials match your filters.</div>
            )}
