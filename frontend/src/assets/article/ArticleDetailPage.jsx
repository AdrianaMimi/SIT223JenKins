...
export default function ArticleDetailPage() {
  ...
  const filledForIndex = (i) => (hoverStar > 0 ? i <= hoverStar : i <= (myRating || 0));
  ...
  async function handleRate(value) {
    if (!user) { showToast('Please log in to rate.', false); return; }
    const v = clamp(value, 1, 5);
    try {
      await runTransaction(db, async (tx) => {
        const artRef = doc(db, 'articles', id);
        const myRef = doc(db, 'articles', id, 'ratings', user.uid);

        const [artSnap, mySnap] = await Promise.all([tx.get(artRef), tx.get(myRef)]);
        ...
        let userSum = a.ratingUserSum != null ? Number(a.ratingUserSum) : Math.max(0, totalSum - seedSum);
        let userCnt = a.ratingUserCount != null ? Number(a.ratingUserCount) : Math.max(0, totalCnt - seedCnt);
      ...
      showToast('Could not submit rating. Please try again.', false);
    }
  }
  ...
  async function handleAdminApplySeed() {
    ...
    try {
      ...
      showToast('Seed rating updated.', true);
    } catch (e) {
      console.error('apply seed failed', e);
      showToast('Failed to apply seed.', false);
    }
  }
  ...
  async function handleAdminResetSeed() {
    ...
    try {
      await runTransaction(db, async (tx) => {
        ...
        const userSum = Number(
          a.ratingUserSum ?? Math.max(0, Number(a.ratingSum || 0) - Number(a.ratingSeedSum || 0))
        );
        const userCnt = Number(
          a.ratingUserCount ?? Math.max(0, Number(a.ratingCount || 0) - Number(a.ratingSeedCount || 0))
        );
      ...
      setSeedAvgEdit('');
      setSeedCountEdit('');
      showToast('Seed reset.', true);
    } catch (e) {
      console.error('reset seed failed', e);
      showToast('Failed to reset seed.', false);
    }
  }
  ...
  async function handleAdminResetUserRatings() {
    ...
    try {
      const ratingsSnap = await getDocs(collection(db, 'articles', id, 'ratings'));
      ...
      setMyRating(null);
      showToast('All user ratings cleared.', true);
    } catch (e) {
      console.error('reset user ratings failed', e);
      showToast('Failed to reset user ratings.', false);
    }
  }
  ...
  useEffect(() => {
    if (!id) return;
    const qy = query(collection(db, 'articles', id, 'comments'), orderBy('createdAt', 'asc'));
  ...
  async function handleAddComment(e) {
    e.preventDefault();
    if (!user) { showToast('Please log in to comment.', false); return; }
    const text = newComment.trim();
    if (!text) return;

    const authorDisplay =
      isAdmin && adminNick.trim()
        ? adminNick.trim()
    ...
    try {
      ...
      showToast('Could not post comment.', false);
    }
  }

  async function handleToggleUpvote(commentId) {
    if (!user) { showToast('Please log in to vote.', false); return; }
    try {
      await runTransaction(db, async (tx) => {
        const cref = doc(db, 'articles', id, 'comments', commentId);
      ...
      showToast('Could not update vote.', false);
    }
  }
  ...
  if (loading) return <div className="container py-4 text-muted">Loading article…</div>;
  if (!docData) return <div className="container py-4">Article not found.</div>;
  ...
  return (
    <div className="container py-4">
      ...
      {/* Body */}
      <article className="mb-5" style={{ fontSize: '1.05rem' }}>
        <ReactMarkdown
